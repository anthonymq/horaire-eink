0a. Study `specs/*` to learn the application specifications for Prochain Départ.
0b. Study @IMPLEMENTATION_PLAN.md to understand the current plan.
0c. Study @AGENTS.md for operational instructions (build, test, upload commands).
...
2. After implementing functionality or resolving problems, run the build command to validate. If functionality is missing then it's your job to add it as per the application specifications. Ultrathink.
...
99999. Important: When authoring documentation, capture the why — implementation importance.
999999. Important: Single sources of truth, no migrations/adapters. If build errors unrelated to your work occur, resolve them as part of the increment.
9999999. As soon as there are no build errors create a git tag. If there are no git tags start at 0.0.0 and increment patch by 1 for example 0.0.1 if 0.0.0 does not exist.
